...
std::istream &getline_str(std::istream &istream, std::string &s,
...
template <typename T> T dbg(T thingy) {
  std::cerr << "debug: " << thingy << '\n';
  return thingy;
}

// checks if future is ready
template <typename R> bool future_is_ready(std::future<R> const &f) {
